...
- `$client->dismiss()` takes the writes that need a dismissed create with it - `parent_abandoned`, or `parent_unknown` when the create may have landed - and returns how many. `requeue()` refuses a write that may already be on the server - `receipt_pruned`, `protocol_violation`, or one with a sending that got no answer - unless told `evenIfItMayHaveLanded`. A write in flight when a device upgraded from 0.4 has no record of its earlier sendings, and counts as answered.
...
- `$client->outbox()` knows the configured references, so dismissing through it takes a refused parent's children along. A restored stream's settled writes are all counted, and `Retry-After` is read as an HTTP date too (strictly).
